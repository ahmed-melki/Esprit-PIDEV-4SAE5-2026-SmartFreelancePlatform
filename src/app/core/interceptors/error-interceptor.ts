...
export class ErrorInterceptor implements HttpInterceptor {
  ...
  intercept(req: HttpRequest<unknown>, next: HttpHandler) {
    return next.handle(req).pipe(catchError((error: HttpErrorResponse) => this.handleError(req, error)));
  }

  private handleError(req: HttpRequest<unknown>, error: HttpErrorResponse) {
    ...
    if (this.errorPages.includes(error.status)) {
      // En dev, un 500 backend sur un endpoint API ne doit pas rediriger toute l'app vers /500.
      if (error.status === STATUS.INTERNAL_SERVER_ERROR && isApiCall) {
